/**
 * Email template for appointment reminder sent to the business owner
 *
 * Tone: informative — the owner is reminded about an upcoming appointment.
 * Includes customer details so the owner can prepare.
 */

export interface BusinessReminderEmailData {
    businessName: string
    customerName: string
...
function getReminderHeader(type: '24h' | '2h'): string {
    return type === '24h' ? 'Turno mañana' : 'Turno en 2 horas'
}

export function renderBusinessReminderEmail(data: BusinessReminderEmailData): string {
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    <tr>
                        <td style="padding: 32px 32px 24px 32px; text-align: center; border-bottom: 1px solid #eaeaea;">
                            <h1 style="margin: 0 0 8px 0; font-size: 24px; font-weight: 600; color: #6366f1;">
                                ⏰ ${headerText}
                            </h1>
                            <p style="margin: 0; font-size: 16px; color: #666666;">
                                ${escapeHtml(data.customerName)} tiene turno en ${escapeHtml(data.businessName)}.
                            </p>
                        </td>
                    </tr>
                    <tr>
...
export function renderBusinessReminderEmailText(data: BusinessReminderEmailData): string {
    const headerText = getReminderHeader(data.reminderType)
    const lines = [
        `⏰ ${headerText.toUpperCase()}`,
        ``,
        `${data.customerName} tiene turno en ${data.businessName}.`,
        ``,
        `DETALLES DEL TURNO`,
        `─────────────────────────`,
