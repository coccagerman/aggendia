...
/**
 * Valida el input para crear un recurso / prestador
 */
export function validateCreateResourceInput(input: CreateResourceInput): void {
    if (!input.name || input.name.trim().length === 0) {
        throw new AppError(ValidationErrorCodes.VALIDATION_ERROR, 'El nombre del recurso / prestador es requerido.', 400)
    }

    if (input.name.trim().length > 100) {
        throw new AppError(
            ValidationErrorCodes.VALIDATION_ERROR,
            'El nombre del recurso / prestador no puede exceder 100 caracteres.',
            400
        )
    }
}
...
/**
 * Valida el input para actualizar un recurso / prestador
 */
...
/**
 * Resultado de la validación para eliminar un recurso / prestador.
 */
...
/**
 * Verifica si un recurso / prestador puede ser eliminado.
...
export function canDeleteResource(_resourceId: string): CanDeleteResult {
    ...
    // la cantidad de turnos futuros para este recurso / prestador
    return {
        canDelete: true,
