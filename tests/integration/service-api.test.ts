...
describe('Services API Integration', () => {
    it('validates Supabase connection is working for services', async () => {
        const supabase = createClient(supabaseUrl, supabaseAnonKey)
        const testEmail = `service-integration-${Date.now()}@example.com`
    ...
    // HTTP API tests for services are covered by E2E tests with Playwright
